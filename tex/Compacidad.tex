...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item $X^*$ es \hausdorff. En efecto, como $\T$ es \hausdorff, tan solo podríamos encontrar problemas a la hora de separar $x\in X$ y $\infty$. Ahora bien, como $X$ es localmente compacto, $x$ tiene un entorno $K$ compacto. Además, $X^*\setminus K$ es abierto en $\T^*$ que contiene a $\infty$.
	...
	\end{enumerate}
\end{proof}
A modo de resumen podemos decir que la compactificación de Alexandroff es realmente compactificación. Continuemos con una pequeña observación, que nos, 
...
\begin{defi}[Compactificación unipuntual]
	Si $X$ es un espacio no compacto e $Y$ es un espacio compacto en el que existe un homeomorfismo $f:X\to f(X)\subset Y$ con $f(X)$ denso en $Y$, es claro que $Y$ es una compactificación de $X$.
	
	No obstante, si además $X$ es \hausdorff, localmente compacto e $Y\setminus X$ es un único punto, se dice que $Y$ es una \tbi[compactificación!unipuntual]{compactificación unipuntual} de $X$.
\end{defi}
Tras esto ya podemos enunciar nuestro teorema de unicidad.
\begin{theo}[Unicidad]
	Sea $X$ un espacio \hausdorff no compacto y $(Y,\T')$ una compactificación unipuntual de $X$, entonces $X^*$ es homeomorfo a la compactificación de Alexandroff de $X$.
\end{theo}
\begin{proof}
	Si $Y$ es una compactificación unipuntual, por definición habrá un homeomorfismo $f:X\to f(X)\subset Y=f(X)\cup\{\omega\}$.
	
	Definimos la aplicación $h : Y \to X^*$ mediante $h(w) = \infty$ y $h(f(x)) = x$ para todo $x \in X$. Veamos que esta aplicación es un homeomorfismo (como $f$ es homeomorfismo está bien definida).
	
	Evidentemente $h$ es biyectiva. Además, al ser $Y$ compacto y $X^*$ ser \hausdorff $h$ es una aplicación cerrada, y por lo tanto homeomorfismo.
...
\end{proof}
Luego el hecho de hacer a un espacio compacto añadiendo un único punto caracteriza a la compactificación de Alexandroff.
