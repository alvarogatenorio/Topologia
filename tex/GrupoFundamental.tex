...
\begin{lem}[Existencia de elevación local]
	...
	\begin{proof}
		...
		Para ver que, en efecto, muere en $U_\lambda$, usamos que $\widetilde{H}(z_0)\in p^{-1}(x_0)$, pues $p\widetilde{H}(z_0)=H(z_0)=x_0$. Entonces, consideramos:
...
\begin{lem}[De elevación de homotopías]
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\[\widetilde{H}_y(y, t_i)\in p^{-1}(U_{x_{i+1}})=\bigcup_\lambda U_\lambda^{x_{i+1}}\]
...
\begin{lem}[De elevación de caminos]
	...
	\begin{proof}
		Si en el lema de elevación de homotopías tomamos $\Y=\{a\}$, y consideramos la función $\widetilde{\sigma_{0}}:\Y\to \widetilde{\X}$ tal que $\widetilde{\sigma_{0}}(a)=\widetilde{x_0}$, tenemos el resultado.
	\end{proof}
\end{lem}
