...
\begin{prop}[Continuidad como propiedad local] Si hay un entorno $V_{x_0}$ de $x_0$ tal que $f\restriction_{V_{x_0}}$ es continua en $x_0$ entonces $f$ es continua en $x_0$.
	
	\begin{proof}
		Sea $V_{f(x_0)}$ un entorno de $f(x_0)$, por continuidad de  $f\restriction_{V_{x_0}}$ en $x_0$ se cumple que $(f\restriction_{V_{x_0}})^{-1}(V_{f(x_0)}) = f^{-1}(V_{f(x_0)})\cap V_{x_0}$ es entorno de $x_0$ en $V_{x_0}$, luego, por topología relativa, $f^{-1}(V_{f(x_0)})$ es entorno de $x_0$ en $\X$ y, por tanto, $f$ es continua en $x_0$.
	\end{proof}
\end{prop}
...
\begin{obs}[Propiedades de los homeomorfismos]\
	\begin{enumerate}
		...
		Que $f$ sea continua es equivalente a que si $U\subset Y$ es abierto, $f^{-1}(U)$ también lo será. Del mismo modo, es equivalente a que si $F\subset Y$ es cerrado, $f^{-1}(F)$ también lo será.
		
		Igualmente, que $f^{-1}$ sea continua es equivalente a que si $F\subset X$ es cerrado, entonces $f(F)$ es cerrado, asimismo si $U\subset X$ es abierto, $f(U)$ también lo será.
		...
		Si una es aplicación no es abierta no tiene por qué ser cerrada (y viceversa) y que si una aplicación es abierta y cerrada no tiene por qué ser biyectiva.
		Vamos a poner una serie de ejemplos que ilustren esto.
		\begin{itemize}
			\item Ejemplo de una aplicación que es abierta y no es continua. Si tomamos $\X = (\R, \T_{usual})$ e $\Y = (\{a, b\}, \T_{discreta})$, tal que $f : \X \rightarrow Y$:\\
			\begin{equation}
				...
				\right.
			\end{equation}
			Es abierta y no continua, ya que $f^{-1}(\{a\})$ no es abierto en $X$ (a su vez también es cerrada y tampoco continua).
			\item Una aplicación que no es abierta ni cerrada. Si tomamos $\X = (\A, \T_{discreta})$ e $\Y = (\A, \T_{trivial})$ , con $\A$ un conjunto cualquiera, y $f$ la identidad.
			\item Una aplicación que es abierta y no es cerrada. Si tomamos $\X = (\{a, b\}, \T_{discreta})$ e $\Y = (\{a, b\}, \T = \{\emptyset, \{a\}, \{a, b\})$, y $f$ la aplicación constante en $a \in \Y$.
			\item Una aplicación que es cerrada y no es abierta. Si tomamos $\X = (\{a, b\}, \T_{discreta})$ e $\Y = (\R, \T_{usual})$, y $f: \X \rightarrow \Y$:\\
			\begin{equation}
			...
			\right.
			\end{equation}
			\item Una aplicación que es abierta y cerrada. Si tomamos un espacio topológico arbitrario $\X = \Y$ y $f$ la aplicación identidad.
			
			Las demostyraciones de estos ejemplos se dejan al lector.
			
		\end{itemize}
		\item Evidentemente, una aplicación biyectiva continua y abierta es un homeomorfismo. Además, un homeomorfismo local es una aplicación abierta (¡compruébese!).\qedhere
		\end{enumerate}
\end{obs}
