...
\label{etop}
La necesidad del estudio de la proximidad y la continuidad de la forma más abstracta posible, es decir, absteniéndose del uso de la noción de distancia, dio origen a la \emph{Topología}.

La idea de \emph{espacio topológico} se comenzó a desarrollar durante los siglos XIX y XX por matemáticos como Fréchet, Kuratowski, Alexandroff y Hausdorff, entre otros. La definición original de estos espacios se puede encontrar en el libro \emph{``Grundzüge der Mengenlehre''}\footnote{\emph{``Teoría abstracta de conjuntos''}, publicado en $1914$ por Félix Hausdorff (1868-1942).}.


Al comienzo de este capítulo introducimos la noción moderna de espacio topológico, añadiendo unos cuantos ejemplos, y posteriormente, presentaremos las propiedades elementales de los conjuntos abiertos y cerrados.
\section{Espacios topológicos. Definición y ejemplos.}
\label{etop_definicionEjemplos}
Comenzamos, como no podía ser de otra manera, definiendo la estructura sobre la que trabajaremos a lo largo de todas estas notas, los espacios topológicos.
\begin{defi}[Espacio topológico]
	\label{etop_def_espacioTopologico}
	Un \tbi[espacio!topológico]{espacio topológico} es un conjunto arbitrario no vacío $\mc{X}$ equipado con una colección $\T$ de subconjuntos $\mc{U}\subset\mc{X}$ que cumplen las siguientes propiedades:
	\begin{enumerate}
		...
		$\bigcup_{i\in I}\U_i\in\T$ donde cada $\U_i\in\T$, siendo $I$ un conjunto arbitrario de índices.
		\item La intersección finita de conjuntos de $\T$ está en $\T$. Dicho de otra forma, $\bigcap_{i=1}^{n}\U_i\in\T$ donde cada $\U_i\in\T$.
	\end{enumerate}
	A menudo hablaremos tan solo de \tbi[espacio!topológico]{espacio} para referirnos a los espacios topológicos.
\end{defi}
...
\begin{obs}[Sutilezas]
	...
	Se desprende de la definición \ref{etop_def_espacioTopologico} que un espacio topológico no es más que un par $(\X, \T)$. Como es natural, salvo que sea necesario, nos referiremos a un espacio topológico por el conjunto que lo conforma, al igual que hacemos en casi todas las ramas de las matemáticas (espacios vectoriales, grupos, anillos,\ldots).
\end{obs}
...
\begin{obs}[Terminología]
	...
	Como es natural, llamaremos \tbi[punto]{puntos} a los elementos de $\X$.
\end{obs}
...
\begin{exa}[Topologías]
	...
	Las demostraciones de que, efectivamente, se cumplen las restricciones impuestas por la definición \ref{etop_def_espacioTopologico} se dejan al lector como ejercicio inmediato.
	\begin{enumerate}
		\item Un \tbi[espacio!métrico]{espacio métrico} es un par $(M,d)$ donde $M$ es un conjunto arbitrario no vacío y $d$ es una \tbi[función!métrica]{métrica}, es decir, una función $d:M\times M\to[0,\infty)$ que verifica:
		\begin{enumerate}
			\item $d(x,y)=0$ si y solo si $x=y$. Esto es, $d$ es una función \tbi[función!definida positiva]{definida positiva}.
			\item $d(x,y)=d(y,x)$. Es decir, $d$ es \tbi[función!simétrica]{simétrica}.
			\item $d(x,y)\leq d(x,z)+d(z,y)$ para todo $x$,$y$,$z\in M$. A esta desigualdad se la conoce con el nombre de \tbi[desigualdad!triangular]{desigualdad triangular}.
		\end{enumerate}
		Llamaremos \tbi[bola]{bola} con centro en un punto $x\in M$ y radio $r\in(0,\infty)$ al conjunto de puntos de $M$ cuya distancia a $x$ es menor que $r$. Escrito formalmente
		\begin{equation}
			\bola_{d}(x,r):=\{y\in M\tq d(x,y)<r\}
		\end{equation}
		Por otra parte, diremos que $\U\subset M$ es un \tbi[conjunto!abierto métrico]{abierto métrico} si para todo punto $x\in\U$ nos es posible encontrar una bola centrada en $x$ que esté contenida en $\U$.
		
		El par $(M,\T_d)$, donde $\T_d$ es la colección de todos los abiertos métricos de $(M,d)$, es un espacio topológico. A esta topología la llamaremos \tbitop[$\T_u$]{usual} o \tbitop[$\T_d$]{inducida por la métrica} $d$.
...
\end{exa}
Para finalizar la sección observamos que en un mismo conjunto $\X$ podemos definir diversas topologías, por ejemplo, podemos equipar a un mismo conjunto $\X$ con la topología discreta y la trivial. Dichas topologías pueden ``compararse'' en determinado contexto, tal y como vemos a continuación.
...
\end{defi}
En lo que resta de capítulo iremos introduciendo algunos conceptos generales de los que haremos uso de forma constante a lo largo del texto.
\section{Conjuntos abiertos e interior}
\label{etop_entornos}
En esta sección introducimos el concepto de \emph{entorno}, cuya utilidad inmediata es caracterizar a los conjuntos abiertos de un espacio topológico $\X$.
\begin{defi}[Entorno de un punto]
	\label{etop_def_entorno}
	Un \tbi{entorno} de un punto $a\in\X$ es un conjunto que contiene a un abierto que contiene al punto $a$. Normalmente denotaremos con la letra $\V$ a los entornos, esta costumbre se debe a un galicismo\footnote{Aunque no estamos muy seguros de cuál.}. Escribamos la definición de entorno \ref{etop_def_entorno} de forma conjuntista para que no quede ninguna duda.
	
	Dado un conjunto $\V\subset \X$, éste se dice entorno de un punto $a\in \X$ si existe un abierto $\U$ de forma que
	\begin{equation}
...
B:=\bigcup_{\W\subset A}\W\text{ con }\W \text{ abierto.}
\end{equation}
...
En esta sección estudiaremos los conjuntos cerrados. Cabe destacar que la noción de ser cerrado no es exactamente la contraria a la de ser abierto, ya que, como veremos más adelante, hay conjuntos que no son ni abiertos ni cerrados, así como conjuntos que son abiertos y cerrados a la vez, los famosos \emph{clopens}.

\begin{defi}[Conjunto cerrado]
	Un conjuto $\F$ de un espacio topológico $\X$ se dice \tbi[conjunto!cerrado]{cerrado} si su complementario, $\X\setminus\F$, es abierto. Usualmente denotaremos a los conjuntos cerrados con las letras $\F$ o $\mc{H}$.
\end{defi}
...
\begin{proof}Vayamos caso por caso.
	\begin{enumerate}
		\item $\X$ es cerrado pues $\X\setminus\X=\emptyset$ es abierto. Asimismo, $\emptyset$ es cerrado, pues $\X\setminus\emptyset=\X$ es abierto.
		...
		\end{equation*}
		que es abierto, por ser la unión arbitraria de abiertos un abierto.
...
\begin{obs}[Abiertos y cerrados a la vez]
	...
	Basta con mirar con atención este lema \ref{etop_lem_propiedadesCerrados} para darse cuenta de que ya hemos encontrado dos conjuntos que son abiertos y cerrados a la vez, el vacío y el total.
	
	Estos conjuntos abiertos y cerrados a la vez (o ``\tbi[clopen]{clopens}'', del inglés close-open) son de vital importancia a la hora de estudiar la noción de \emph{conexión}, como veremos más adelante. 
\end{obs}
...
Vamos a desgranar ahora una serie de resultados que nos van a hacer ver que la adherencia y el interior de un conjunto son, de alguna manera, conceptos duales.
...
\begin{obs}[Adherencia y conjunto]
	...
	A\subset\adher{A}
	\end{equation}
	Esto es debido a que, evidentemente, cualquier entorno de un punto $a\in A$ contiene al punto $a$, luego, por definición, corta al conjunto $A$.
	
	Visto en perspectiva, esta desigualdad es de alguna manera la dual a la \eqref{etop_eq_desigualdadInterior1}. Además, combinando ambas desigualdades obtenemos que un conjunto siempre queda ``ensandwichado'' entre su interior y su adherencia.
	\begin{equation*}
...
Es fácil darse cuenta de que, como la intersección arbitraria de cerrados es un cerrado, el menor conjunto cerrado que contiene a uno dado puede ser construido coleccionando todos los cerrados que contienen a dicho conjunto y cortándolos. Es decir
\begin{equation}
\label{etop_eq_menorCerrado}
B:=\bigcap_{\mc{H}\supset A}\mc{H} \text{ con }\mc{H} \text{ cerrado.}
\end{equation}
En efecto es un conjunto que contiene a $A$ ya que todos los conjuntos de la familia a intersecar contienen a $A$, además, es el menor de ellos, ya que, de haber uno más pequeño, pertenecería a la familia que se está intersecando, lo cual es absurdo.
...
\begin{proof}
	...
	\begin{enumerate}
		\item[\bsubset] 
		Dado $z\in \adher{A\cup B}$, todo entorno $\V_z$ de $z$ corta a $A\cup B$. Veamos que $z$ es, o bien adherente a $A$, o bien adherente a $B$ (quizá a ambos). Para ello supondremos que no es adherente a ninguno de ellos, es decir, que existe un entorno $\W_z$ de $z$ que no corta ni a $A$ ni a $B$. Si este entorno existiera tampoco cortaría a la unión, lo cual es absurdo.
		\item[\bsupset]
...
\begin{obs}[Demostración alternativa]
	Una demostración alternativa y quizá más contundente (psicológicamente hablando) de la proposición \ref{etop_prop_unionAdher} consistiría en usar las propiedades de distributividad de las operaciones conjuntistas.
	
	En efecto, $z\in\adher{A\cup B}$ si y solo si todo entorno abierto\footnote{Usualmente, en vez de decir que $\U$ es un entorno abierto de un punto $x\in\X$, diremos simplemente que $\U$ es un abierto de $x$.} $\U$ de $z$ cumple que
	\begin{equation*}
...
\begin{cor}[Intersección de interiores]
	El interior de la intersección es la intersección de los interiores.
	\begin{equation*}
...
\begin{exa}[Cuadrados abiertos]
	...
	Veámoslo con un dibujo. En la parte izquierda de la figura \ref{etop_img_cuadrados} vemos los conjuntos $A$ y $B$, cuya intersección es vacía. En la parte derecha de la figura quedan representadas sus adherencias, cuya intersección no es vacía, quedando esta marcada con una línea roja continua.
\end{exa}	
\begin{figure}[h!]
	\centering
	\includegraphics[scale = 0.2]{img/intersecciones}
	\caption{Ilustración del ejemplo \ref{etop_ejem_cuadradosAbiertos}}
	\label{etop_img_cuadrados}
\end{figure}
...
En esta sección presentamos el concepto de \emph{punto de acumulación}, que es muy útil para trabajar con sucesiones, como veremos más adelante. Además, definiremos la idea de que un conjunto sea \emph{denso} de varias maneras que nos serán muy útiles a la hora de resolver problemas.
\begin{defi}[Punto de acumulación]
	\label{etop_defi_puntoAcumulacion}
	Dado un conjunto $A$ en un espacio topológico $\X$, se dice que un punto $x\in\X$ es un \tbi[punto!de acumulación]{punto de acumulación} de $A$ si todo entorno $\V_x$ de $x$ verifica que
	\begin{equation*}
...
\begin{exa}[Espacios separables]
	...
	\begin{enumerate}
		\item $\Q^n$ es un conjunto denso y numerable en $\R^n$ con la topología usual. La numerabilidad de $\Q^n$ es consecuencia de la numerabilidad de $\Q$ (recuérdese que el producto numerable de conjuntos numerables es numerable). Probar la densidad de $\Q^n$ ya es un poco más engorroso y se deja al lector como ejercicio.
