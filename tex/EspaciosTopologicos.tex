...
La definición inicial de estos espacios se puede encontrar en el libro \tbi{``Grundzüge der Mengenlehre''}\footnote{``Teoría abstracta de conjuntos'', publicado en $1914$ por \tbi{Félix Hausdorff}(1868-1942).} publicado por este último autor.
...
\begin{obs}[Terminología]
	...
	Asimismo, los conjuntos que conforman $\T$ reciben el nombre de \tbi{abiertos} de $\X$. Normalmente los denotaremos con las letras $\U$ o $\W$.
...
\begin{defi}[Punto de Acumulación]
	...
	Dado un conjunto $A$ en un espacio topológico $\X$, se dice que un punto $x\in\X$ es un \tbi{punto de acumulación} de $A$ si todo entorno de $\V_x$ de $x$ verifica que
	\begin{equation*}
...
\begin{defi}[Conjunto Derivado]
	Al conjunto de los puntos de acumulación de un conjunto $A$ se le denomina \tbi{conjunto derivado}. Usualmente denotaremos al conjunto derivado por $A'$.
\end{defi}
\begin{obs}[Entornos Perforados]
	...
	Por ejemplo $\V_x\setminus\{x\}$. A este último conjunto se le suele denominar \tbi{entorno perforado} o \tbi{entorno pinchado} de $x$.
\end{obs}
...
\begin{defi}[Puntos Aislados]
	Se llaman \tbi{puntos aislados} de un conjunto $A$, a aquellos puntos de $A$ que no son puntos de acumulación. Es decir, a los puntos $x\in A$ que poseen un entorno que cumple
	\begin{equation*}
		\V_x\cap A = \{x\}
	\end{equation*}
\end{defi}
...
\begin{defi}[Conjunto Denso]
	...
	Decimos que un conjunto $A$ es \tbi{denso} en un espacio topológico $\X$ si la adherencia de $A$ es el propio espacio $\X$. Es decir
	\begin{equation*}
		\adher{A}=\X
	\end{equation*}
\end{defi}
...
\begin{defi}[Espacio Topológico Separable]
	Se dice que un espacio topológico $\X$ es \tbi{separable} si posee un subconjunto numerable denso.
\end{defi}
...
\begin{defi}[Base de entornos]
	...
	%Rellenable en el futuro
\end{defi}

\section{Base de abiertos}
\begin{defi}[Base de abiertos]
	\label{etop_bases_abiertos}
	Una \tbi{base} de $\T$ es una colección $\B$ de abiertos, tal que todo abierto de $\T$ es unión de abiertos de $\B$
\end{defi}
...
\begin{defi}[II axioma de numerabilidad]
	...
	Diremos que $\X$ es \tbi{II axioma} (de numerabilidad) si tiene una base numerable.
\begin{exa}
	ejemmplos...%Por hacer
\end{exa}
\end{defi}
