...
\begin{exa}[Topologías]
	...
	\begin{enumerate}
		...
		El par $(M,\T_d)$, donde $\T_d$ es la colección de todos los abiertos métricos de $(M,d)$, es un espacio topológico. A esta topología la llamaremos \tbitop[$\T_d$]{usual} o inducida por la métrica $d$.
...
\end{exa}
	...
	Veamoslo con un dibujo. En el vemos en línea discontinua los conjuntos $A$ y $B$ en el de la izquierda, y cuya intersección dará vacía, y a la derecha sus adherencias, cuya intersección no es vacía, marcada con una línea roja continua.
	
	\begin{figure}[h!]
		\centering
		\includegraphics[scale = 0.20]{img/Intersecciones}
		\caption{En el de la izquierda se ve $\adher{A\cap B}=\emptyset$ mientras que la de la derecha representa $\adher{A}\cap\adher{B}=\{1\}\times[0,1]$}
	\end{figure}
