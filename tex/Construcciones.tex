...
Es hora de profundizar un poco más. Tomemos otro espacio topológico arbitrario $(\mathcal{Z},\T')$ y consideremos una función $g$ que nace en este y muere en $\Y$.
...
Desde luego, la proposición \ref{const_universalInmersiones} nos dice que para la topología $f^{-1}(\T)$ se verifica la propiedad universal (y que el la menos fina que lo hace, pues es la menos fina que hace a $f$ continua).

Ahora vamos a ver que no solo ocurre esto sino que el la única topología que verifica esta propiedad universal.
...
\begin{proof}
	...
	Para la otra implicación consideramos un diagrama análogo, pero tomando $(Z,\T')=(\Y,\T_\Y)$ y $g=\text{id}$.  
...
Sin embargo, esto no termina aquí, ya que $f$ es \tb{abierta}. En efecto, dado un abierto $\W\subset\Y$, entonces existe un abierto $\U\in\T$ de modo que $f(\W)=\U\cap f(\Y)$. En efecto, 

\[f(\W)=f(f^{-1}(\U))=f(f^{-1}(U\cap f(\Y)))=\U\cap f(\Y),\]

que es un abierto de $f(\mathcal{\Y})$ (la segunda igualdad se deduce de que $f$ es inyectiva). Por tanto, al ser $f$ continua y abierta, es \tb{homeomorfismo}. Esto es sorprendente, ya que una aplicación inyectiva de $\mathcal{\Y}$ en $\mathcal{\X}$ es homeomorfismo de $\mathcal{\Y}$ en $f(\mathcal{\Y})$.
...
\end{equation}
Lo que buscamos en la construcción del espacio producto es de dotar a $Y$ de la topología $\T$ más fina haga continuas todas las aplicaciones $p_i^r$ para $1\le i\le r$. A dichas  $p_i^r$ las conoceremos a partir de ahora como \tbi[proyeccion@proyección]{proyecciones}.
