...
\end{equation}
Lo que buscamos en la construcción del espacio producto es dotar a $Y$ de la topología $\T$ más fina que haga continuas todas las aplicaciones $p_i^r$ para $1\le i\le r$. A dichas  $p_i^r$ las conoceremos a partir de ahora como \tbi{proyecciones}.
...
\begin{proof}
	...
	%	(Y,\T_Y) \ar[p_{r_i}]^{p_i} & 
	%	(X_i,\T_i) \\
	%	(Y,\T_Y) \ar[u]^{id} \ar[ru]_{p_{r_i} = p_{r_i} \circ id} &
	%}\]
	...
	%	(Y,\T_Y) \ar[p_{r_i}]^{p_i} & 
	%	(X_i,\T_i) \\
	%	(Y,\prod\T_i) \ar[u]^{id} \ar[ru]_{p_{r_i} = p_{r_i} \circ id} &
	%}\]	
